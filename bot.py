...
IDS_PERMITIDOS = [1249477991747293266, 814220712398684261]

# ID do dono do bot
ID_DONO = 1249477991747293266
...
async def on_ready():
    ...
    await bot.change_presence(
        ...
        activity=discord.Game(name="Use com sabedoria!!")
    )
    try:
...
async def dar_permissao(interaction: discord.Interaction, usuario: discord.Member):
    if not await verificar_se_dono(interaction):
        return
...
async def grudar(interaction: discord.Interaction, usuario: discord.Member):
    ...
    if interaction.user.voice and interaction.user.voice.channel:
        try:
            await usuario.move_to(interaction.user.voice.channel)
...
if __name__ == "__main__":
    token = input("Insira o token do bot: ")
    bot.run(token.strip())
